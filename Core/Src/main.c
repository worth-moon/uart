...
void Error_Handler(void)
{
  ...
  while (1)
  {
		HAL_UART_Transmit(&huart1, (const uint8_t *)"RUN Error_Handler", strlen((const char *)"RUN Error_Handler"), 1000);
		HAL_Delay(1000);
  }
...
void assert_failed(uint8_t *file, uint32_t line)
{
	...
	sprintf((char *)error_buffer, (const char *)"Wrong parameters value: file %s on line %d\r\n", file, line);
	HAL_UART_Transmit(&huart1, error_buffer, strlen((const char *)error_buffer), 1000);
  /* USER CODE BEGIN 6 */
